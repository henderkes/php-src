                ...
                case "DEFINES":
                    ...
                    switch ($kind) {
                        case ZEND_VM_KIND_HYBRID:
                            ...
                            out($f,"#  ifdef ZEND_VM_HOLD_EG_IN_REG\n");
                            out($f,"   /* r13 holds the executor-globals pointer for the engine VM and must not be clobbered */\n");
                            out($f,"#   define HYBRID_JIT_GUARD() __asm__ __volatile__ (\"\"::: \"rbx\",\"r12\")\n");
                            out($f,"#  else\n");
                        ...
                        case ZEND_VM_KIND_CALL:
                            ...
                            out($f,"#else\n");
                            out($f,"# ifdef ZTS\n");
                            out($f,"#  define ZEND_OPCODE_HANDLER_ARGS zend_execute_data *execute_data, const zend_op *opline, void *_tsrm_ls_cache\n");
                            out($f,"#  define ZEND_OPCODE_HANDLER_ARGS_PASSTHRU execute_data, opline, _tsrm_ls_cache\n");
                            out($f,"# else\n");
...
ENDNAMES;
    ...
    $str .= "# ifdef ZTS\n";
    $str .= "typedef const struct _zend_op *(ZEND_OPCODE_HANDLER_CCONV *zend_vm_opcode_handler_t)(struct _zend_execute_data *execute_data, const struct _zend_op *opline, void *tsrm_ls_cache);\n";
    $str .= "typedef const struct _zend_op *(ZEND_OPCODE_HANDLER_FUNC_CCONV *zend_vm_opcode_handler_func_t)(struct _zend_execute_data *execute_data, const struct _zend_op *opline, void *tsrm_ls_cache);\n";
    $str .= "# else\n";
