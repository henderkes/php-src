...
/* Topmost fixed slot in the reserved fast space, just past the language scanner
 * globals. Claimed first (start_memory_manager() is the first fast resource), so
 * its placement fixes where the ordinary bump-allocated fast resources begin and
 * AG() avoids the runtime offset load. See zend_globals.h for the full layout. */
