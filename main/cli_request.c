...
PHPAPI cli_shell_callbacks_t *php_cli_get_shell_callbacks(void)
{
...
PHPAPI ssize_t sapi_cli_single_write(const char *str, size_t str_length)
{
	ssize_t ret;
...
void sapi_cli_ini_defaults(HashTable *configuration_hash)
{
	zval tmp;
...
#if defined(PHP_WIN32) && defined(HAVE_LIBEDIT) && !defined(COMPILE_DL_READLINE)
		if (!interactive) {
